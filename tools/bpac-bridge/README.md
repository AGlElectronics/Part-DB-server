...
`width_mm` is the tape **length** (Part-DB width). `height_mm` is the tape **width** (12, 18, or 24). `layout` is `stacked` (name under the QR), `beside` (name to the right of the QR), or `text` (description only, cut off at the label length). Physical length is never shorter than 24.5 mm — that is the PT-P700 cutter minimum.
